...
<{{ $tag }} @if ($href) href="{{ $href }}" @endif {{ $attributes->class('inline-flex min-w-0 items-center gap-2 text-ink') }}>
    ...
    @else
        <span aria-hidden="true" class="tag-shape figures inline-flex h-9 shrink-0 items-center bg-brand pe-2.5 font-display text-base font-bold text-white">{{ $shop->initials() }}</span>
