...
it('ignores quantity changes for items that are not in the bag', function () {
    ...
    expect(app(Bag::class)->quantityOf('BB-LIP-1'))->toBe(0);
});
...
it('blocks checkout when a line is no longer available', function () {
    // The shelf held five when this went in the bag, and three by the time the
    // customer came back to it.
    variantFor('GL-VITC-1')->forceFill(['stock_quantity' => 5])->save();
    fillCart(['GL-VITC-1' => 5]);
    variantFor('GL-VITC-1')->forceFill(['stock_quantity' => 3])->save();
